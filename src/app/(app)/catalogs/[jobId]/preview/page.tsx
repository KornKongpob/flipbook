        ...
        <div className="rounded-xl border border-line bg-card p-4 shadow-sm">
          ...
          <div className="space-y-1.5">
            {previewItems.map((item) => (
              ...
              >
                {/* Row: order + thumbnail + name + controls */}
                <div className="flex items-center gap-2">
                  ...
                  {/* Thumbnail */}
                  <div className="relative flex size-9 shrink-0 items-center justify-center overflow-hidden rounded-md border border-line bg-background">
                    {item.previewUrl ? (
                      ...
                      <svg className="size-4 text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909M3 3l18 18" />
                      </svg>
                    )}
                  </div>
                  ...
                  <div className="flex gap-1">
                    <form action={moveItemAction}>
                      ...
                      <button type="submit" title="Move up" className="flex size-6 items-center justify-center rounded-lg border border-line bg-white/80 text-muted transition hover:border-brand/30 hover:bg-brand-soft hover:text-brand">
                    ...
                    <form action={moveItemAction}>
                      ...
                      <button type="submit" title="Move down" className="flex size-6 items-center justify-center rounded-lg border border-line bg-white/80 text-muted transition hover:border-brand/30 hover:bg-brand-soft hover:text-brand">
                    ...
                    <form action={toggleItemVisibilityAction}>
                      ...
                      <button type="submit" title={item.is_visible ? "Hide product" : "Show product"} className={`flex size-6 items-center justify-center rounded-lg border transition ${item.is_visible ? "border-line bg-white/80 text-muted hover:border-rose-200 hover:bg-rose-50 hover:text-rose-500" : "border-emerald-200 bg-emerald-50 text-emerald-600 hover:bg-emerald-100"}`}>
                        {item.is_visible ? (
                ...
                <form action={saveDisplayNameAction} className="mt-2 flex gap-1.5">
                  <input type="hidden" name="jobId" value={jobId} />
                  <input type="hidden" name="itemId" value={item.id} />
                  <Input name="displayName" defaultValue={item.display_name_override ?? ""} placeholder="Override display name…" className="h-8 text-xs" />
                  <Button variant="secondary" className="h-8 shrink-0 px-3 text-xs">Save</Button>
                </form>

                {/* Image upload for items with no image */}
                {!item.previewUrl && (
                  <form
                    action={`/api/items/${item.id}/upload`}
                    method="post"
                    encType="multipart/form-data"
                    className="mt-2 flex items-center gap-1.5 rounded-md border border-dashed border-amber-300 bg-amber-50 px-2 py-1.5"
                  >
                    <input type="hidden" name="jobId" value={jobId} />
                    <input type="hidden" name="returnTo" value="preview" />
                    <input type="hidden" name="saveManualMapping" value="on" />
                    <svg className="size-3.5 shrink-0 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5" />
                    </svg>
                    <input name="asset" type="file" accept="image/png,image/jpeg,image/webp" required className="min-w-0 flex-1 text-[11px] text-muted file:mr-1.5 file:rounded file:border-0 file:bg-amber-500 file:px-2 file:py-0.5 file:text-[11px] file:font-medium file:text-white" />
                    <Button variant="secondary" className="h-6 shrink-0 px-2 text-[11px]">Upload</Button>
                  </form>
                )}
              </div>
            ))}
